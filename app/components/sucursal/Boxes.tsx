        ...
        {boxes.map((box, idx) => (
            <div
                ...
                className={`relative rounded-xl shadow-lg flex flex-col items-center justify-center h-28 md:h-40 transition-all
                            ...
                            ${boxSeleccionado?._id === box._id ? "bg-pink-300 text-pink-800" : ""}
                        `}
                onClick={() => handleBoxSeleccionado(box)}
                data-box-id={box._id}
            >
                {boxLibre(box) ? <LuDoorOpen className={`text-4xl mb-2 ${boxSeleccionado?._id === box._id ? "text-white" : "text-[#d5c7aa]"}`} /> : <LuDoorClosed className="text-4xl mb-2 text-white" />}
                <div className="font-bold text-lg mb-1">Box {box.numero}</div>
                {boxLibre(box) ? (
                    <div className={`text-sm mt-4 ${boxSeleccionado?._id === box._id ? "text-white font-bold" : "text-gray-400"}`}>Libre</div>
                ) : (
                    <>
